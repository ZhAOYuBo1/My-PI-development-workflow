...
const OPENAI_RESPONSES_APIS = new Set<Api>(["openai-responses", "azure-openai-responses"]);
...
function composeTemperatureSanitizer(
	...
	const nextOptions: SimpleStreamOptions = options ? { ...options, temperature: undefined } : {};

	nextOptions.onPayload = async (payload, payloadModel) => {
		const transformedPayload = existingOnPayload ? await existingOnPayload(payload, payloadModel) : undefined;
