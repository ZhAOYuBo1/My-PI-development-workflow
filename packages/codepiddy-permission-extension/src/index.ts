...
import { openPermissionSystemSettingsModal } from "./config-modal.ts";
import { registerModelOptionCompatibilityGuard } from "./model-option-compatibility.ts";
import {
...
} from "./before-agent-start-cache.ts";
import {
...
import { evaluatePermission, type PatternPermissionRule } from "./evaluate-permission.ts";
import {
...
import { isNodeErrorWithCode } from "./jsonc-config.ts";
import { createPermissionSystemLogger, safeJsonStringify } from "./logging.ts";
import {
...
} from "./permission-prompts.ts";
import { SessionApprovalStore } from "./session-approval-store.ts";
import { findSkillPathMatch, resolveSkillPromptEntries, type SkillPromptEntry } from "./skill-prompt-sanitizer.ts";
import { PERMISSION_SYSTEM_STATUS_KEY, syncPermissionSystemStatus } from "./status.ts";
import { sanitizeAvailableToolsSection } from "./system-prompt-sanitizer.ts";
import { checkRequestedToolRegistration, getToolNameFromValue } from "./tool-registry.ts";
import type { PermissionCheckResult } from "./types.ts";
import { canResolveAskPermissionRequest, shouldAutoApprovePermissionState } from "./yolo-mode.ts";
import {
...
function writeLogEntry(stream: "debug" | "review", event: string, details: Record<string, unknown> = {}): void {
	const warning = stream === "debug" ? extensionLogger.debug(event, details) : extensionLogger.review(event, details);
	if (warning) {
...
function isLikelyFilesystemToolName(toolName: string): boolean {
	...
	return FILESYSTEM_TOOL_NAME_SUFFIXES.some((suffix) => normalized.endsWith(suffix) || nameParts.includes(suffix));
}
...
function inferSkillEntryFromReadPath(
	...
	const candidateSkillRoots = [join(PI_AGENT_DIR, "skills"), join(cwd, ...SKILLS_DIR_PARTS)];
...
function createDuplicatePermissionPromptDecision(decision: PermissionPromptDecision): PermissionPromptDecision {
	return decision.approved ? { approved: true, state: "approved" } : clonePermissionPromptDecision(decision);
}
...
function createConfigEvaluationRule(result: PermissionCheckResult): PatternPermissionRule {
	const canReuseMatchedPattern =
		result.source === "bash" || result.source === "mcp" || result.source === "skill" || result.source === "special";
	return {
	...
	for (const { requestPath, request } of pendingRequests) {
		...
		if (requestAgeMs >= PERMISSION_FORWARDING_TIMEOUT_MS) {
			...
			try {
				...
				const timeoutMs =
					forwardedPromptTimeoutSeconds !== null && forwardedPromptTimeoutSeconds > 0
				...
				decision = await requestPermissionDecisionFromUi(
					...
					timeoutMs !== undefined
						? {
								timeoutMs,
								timeoutDenialReason: timeoutDenialReason ?? DEFAULT_FORWARDED_PERMISSION_PROMPT_TIMEOUT_REASON,
							}
						: {},
				);
			} catch (error) {
...
export default function piPermissionSystemExtension(pi: ExtensionAPI): void {
	...
	const applyExtensionConfigSideEffects = (result: PermissionSystemConfigLoadResult, ctx?: ExtensionContext): void => {
		if (ctx) {
			runtimeContext = ctx;
		}
	...
	let modelOptionCompatibilityRegistered = false;
	const ensureModelOptionCompatibilityRegistered = async (): Promise<void> => {
		if (modelOptionCompatibilityRegistered) return;
		try {
			registerModelOptionCompatibilityGuard(pi);
		} catch {
			// Optional compatibility guard failures must not disable permission enforcement.
		}
		modelOptionCompatibilityRegistered = true;
	};
	...
	const emitPermissionStateEvent = (details: PermissionPromptDetails, state: PermissionRequestState): void => {
	...
	const reviewPermissionDecision = (event: string, details: PermissionPromptDetails): void => {
	...
	const promptPermission = async (
		...
		const decisionPromise = (async (): Promise<PermissionPromptDecision> => {
			...
			reviewPermissionDecision(decision.approved ? "permission_request.approved" : "permission_request.denied", {
				...
				decisionScope: getPermissionDecisionScope(details),
				approvalScope:
					decision.approved && decision.state === "always" ? getPermissionDecisionScope(details) : undefined,
			});
	...
	const runForwardedPermissionRequestScan = (): void => {
		...
		void processForwardedPermissionRequests(currentContext, { preserveLocation: true }).finally(() => {
	...
	pi.on("before_agent_start", async (event: BeforeAgentStartEvent, ctx: ExtensionContext) => {
		...
		if (
			!shouldApplyCachedAgentStartState(lastPromptStateCacheKey, promptStateCacheKey) &&
	...
	pi.on("tool_call", async (event: ToolCallEvent, ctx: ExtensionContext) => {
		...
		if (isToolCallEventType("read", event)) {
			...
			if (readPath) {
				...
				const inferredSkill = matchedSkill ? null : inferSkillEntryFromReadPath(readPath, ctx.cwd, "ask");
				...
				const readSkill =
					matchedSkill ??
					(inferredSkill && inferredSkillCheck ? { ...inferredSkill, state: inferredSkillCheck.state } : null);
		...
		const input =
			ctx.cwd &&
			(getNonEmptyString(inputRecord.path) || getNonEmptyString(inputRecord.file_path)) &&
