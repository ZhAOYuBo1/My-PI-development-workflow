...
import { toRecord } from "./common.ts";
import { formatJsoncConfigLoadWarning, isNodeErrorWithCode, parseJsoncConfig } from "./jsonc-config.ts";
...
export function ensurePermissionSystemConfig(configPath = getPermissionSystemConfigPath()): {
	created: boolean;
	warning?: string;
} {
 ...
 */
function readExistingConfig(configPath: string): { record: Record<string, unknown> | null; parseError: boolean } {
	if (!existsSync(configPath)) {
