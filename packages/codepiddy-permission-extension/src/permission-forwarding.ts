...
import { getNonEmptyString, normalizeAgentName } from "./common.ts";
import type { PermissionDecisionState } from "./permission-dialog.ts";
...
export function resolvePermissionForwardingRootDir(options: {
	...
	const explicitAgentDir = normalizeAgentName(env[PERMISSION_FORWARDING_AGENT_DIR_ENV_KEY]);
	...
	const agentDir =
		explicitAgentDir ?? delegatedRuntimeAgentDir ?? routerSharedAgentDir ?? policyAgentDir ?? options.defaultAgentDir;
...
export function resolvePermissionForwardingTargetSessionId(options: {
	...
	return normalizePermissionForwardingSessionId(options.env?.[SUBAGENT_PARENT_SESSION_ENV_KEY]);
}
