import { getNonEmptyString, toRecord } from "./common.ts";
import { safeJsonStringify } from "./logging.ts";
import type { SkillPromptEntry } from "./skill-prompt-sanitizer.ts";
...
function formatEditReference(value: unknown): string {
	return typeof value === "string" && value.trim() ? sanitizeInlineText(value, 40) : "anchor";
}

function formatEditReferenceRange(edit: Record<string, unknown>): string {
	const start = formatEditReference(edit.pos);
	const end = typeof edit.end === "string" && edit.end.trim() ? ` through ${formatEditReference(edit.end)}` : "";
	return `${start}${end}`;
}
