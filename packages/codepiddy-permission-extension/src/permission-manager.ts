...
import { formatJsoncConfigLoadWarning, parseJsoncConfig } from "./jsonc-config.ts";
import type {
	AgentPermissions,
	BashPermissions,
...
function defaultGlobalConfigPath(): string {
	return join(defaultPolicyAgentDir(), "pi-permissions.jsonc");
}
function defaultAgentsDir(): string {
	return join(defaultPolicyAgentDir(), "agents");
}
function defaultLegacyGlobalSettingsPath(): string {
	return join(defaultPolicyAgentDir(), "settings.json");
}
function defaultGlobalMcpConfigPath(): string {
	return join(defaultPolicyAgentDir(), "mcp.json");
}
...
function findLatestTrustedPermissionMatch(
	...
	return findLatestTrustedPatternMatching(patterns, (pattern) => (pattern.regex.test(name) ? name : null));
}
...
export class PermissionManager {
	...
	getToolPermission(toolName: string, agentName?: string): PermissionState {
		...
		return toolMatch?.state ?? resolveLayeredDefaultPermission(layers, "tools")?.state ?? DEFAULT_POLICY.tools;
	}

	checkPermission(toolName: string, input: unknown, agentName?: string): PermissionCheckResult {
		const { merged, layers, compiledTools, compiledSpecial, compiledSkills, compiledMcp, compiledBash } =
		...
		if (normalizedToolName === "skill") {
			...
			if (typeof skillName === "string") {
				...
				return {
					toolName,
					state:
						result?.state ?? resolveLayeredDefaultPermission(layers, "skills")?.state ?? DEFAULT_POLICY.skills,
					matchedPattern: result?.matchedPattern,
					source: "skill",
				};
			}
		...
		if (BUILT_IN_TOOL_PERMISSION_NAMES.has(normalizedToolName)) {
			const result = findCompiledPermissionMatchByPatternOrderForNames(compiledTools, [
				...createActionResourceTargets(normalizedToolName, input),
				normalizedToolName,
			]);
			return {
				toolName,
				state: result?.state ?? resolveLayeredDefaultPermission(layers, "tools")?.state ?? DEFAULT_POLICY.tools,
				matchedPattern: result?.matchedPattern,
				target: result?.matchedName,
				source: "tool",
			};
